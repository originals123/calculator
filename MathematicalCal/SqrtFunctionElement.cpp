...
SqrtFunctionElement::SqrtFunctionElement() {
    this->setFunctionType(FLAG_SQRT);
}

string SqrtFunctionElement::toString() {
    return StringManupulator::getSingleArg(this->arguments[0], OP_SQRT);
}



// throws an exception if number of aruments are greater than 1
void SqrtFunctionElement::addArgument(FormulaElement *e) {
...
double SqrtFunctionElement::evaluate() {
    ...
    if(arg_type == FLAG_CONSTANT) {
        ...
        return sqrt(conObj->evaluate());
    }

    // arg = Variable
    if(arg_type == FLAG_VARIABLE) {
        VariableElement *varObj = dynamic_cast<VariableElement*>(arg);
        return sqrt(varObj->evaluate());
    }

    // arg = Function
    if(arg_type == FLAG_FUNCTION) {
        FunctionElement *funObj = dynamic_cast<FunctionElement*>(arg);
        return sqrt(FunctionEvaluator::evaluateFunction(funObj));
    }
}

void SqrtFunctionElement::setVariableValue(string name, double value) {
